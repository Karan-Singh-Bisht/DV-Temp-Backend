...
exports.syncContacts = async (req, res) => {
  ...
  if (!contacts || !Array.isArray(contacts)) {
    return res
      .status(400)
      .json({ error: "Invalid input: contacts must be an array." });
  }
  ...
  try {
    const contactPromises = contacts.map(async (contact) => {
      ...
      if (existingContact) {
        console.log(
          `Contact ${contact.name} already exists for this user. Skipping...`
        );
        return null;
      }
...
exports.searchByName = async (req, res) => {
  ...
  if (!searchTerm) {
    return res
      ...
      .json({
        error:
          "Search query parameter (name, username, or phone number) is required.",
      });
  }
  ...
  try {
    ...
    for (const user of users) {
      const isInContacts = result.some(
    ...
    if (filteredResult.length === 0) {
      return res
        .status(404)
        .json({ message: "No matching users or contacts found." });
    }
    ...
    return res
      .status(500)
      .json({ error: "Failed to search by name, username, or phone number." });
  }
};
...
exports.searchByPhoneNumber = async (req, res) => {
  ...
  if (!searchTerm) {
    return res.status(400).json({ error: 'Search query parameter (phone number) is required.' });
  }
  ...
  try {
    ...
    // Search in Users collection with phone number
    const users = await User.find({
      phoneNumber: { $regex: `^${normalizedSearchTerm}$`, $options: 'i' }
    });
    ...
    const filteredResult = result.filter(
      (item, index, self) =>
        index === self.findIndex((t) => t.phoneNumber === item.phoneNumber && t.name === item.name)
    );
    ...
    return res.status(500).json({ error: 'Failed to search by phone number.' });
  }
};
